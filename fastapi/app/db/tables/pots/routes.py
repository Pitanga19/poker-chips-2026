...
from app.db.tables.pots.crud import pot_crud as crud
from app.db.tables.pots.schemas import PotRead, PotCreate, PotOptional
from app.core.schemas import DeleteResponse

router = APIRouter(prefix='/pots',tags=['Pots'])

@router.post('/', response_model=PotRead, status_code=201)
async def create_pot_endpoint(data: PotCreate, db: AsyncSession = Depends(get_db)):
    return await crud.create(data, db)

@router.get('/by-id/{id}', response_model=PotRead)
async def get_by_id_endpoint(id: int, db: AsyncSession = Depends(get_db)):
    return await crud.get_by_id(id, db)

@router.get('/filtered', response_model=List[PotRead])
...
@router.patch('/{id}', response_model=PotRead)
async def update_endpoint(id: int, data: PotOptional, db: AsyncSession = Depends(get_db)):
...
async def delete_endpoint(id: int, db: AsyncSession = Depends(get_db)):
    ...
    return {'msg': 'Pozo eliminado correctamente'}
