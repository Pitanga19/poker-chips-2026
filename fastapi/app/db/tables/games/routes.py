...
from app.db.tables.games.crud import game_crud as crud
from app.db.tables.games.schemas import GameRead, GameCreate, GameOptional
from app.core.schemas import DeleteResponse

router = APIRouter(prefix='/games',tags=['Games'])

@router.post('/', response_model=GameRead, status_code=201)
async def create_game_endpoint(data: GameCreate, db: AsyncSession = Depends(get_db)):
    return await crud.create(data, db)

@router.get('/by-id/{id}', response_model=GameRead)
async def get_by_id_endpoint(id: int, db: AsyncSession = Depends(get_db)):
    return await crud.get_by_id(id, db)

@router.get('/filtered', response_model=List[GameRead])
...
@router.patch('/{id}', response_model=GameRead)
async def update_endpoint(id: int, data: GameOptional, db: AsyncSession = Depends(get_db)):
...
async def delete_endpoint(id: int, db: AsyncSession = Depends(get_db)):
    ...
    return {'msg': 'Juego eliminado correctamente'}
