...
from app.db.tables.seats.crud import seat_crud as crud
from app.db.tables.seats.schemas import SeatRead, SeatCreate, SeatOptional
from app.core.schemas import DeleteResponse

router = APIRouter(prefix='/seats',tags=['Seats'])

@router.post('/', response_model=SeatRead, status_code=201)
async def create_seat_endpoint(data: SeatCreate, db: AsyncSession = Depends(get_db)):
    return await crud.create(data, db)

@router.get('/by-id/{id}', response_model=Optional[SeatRead])
async def get_by_id_endpoint(id: int, db: AsyncSession = Depends(get_db)):
    return await crud.get_by_id(id, db)

@router.get('/filtered', response_model=List[SeatRead])
...
@router.patch('/{id}', response_model=Optional[SeatRead])
async def update_endpoint(id: int, data: SeatOptional, db: AsyncSession = Depends(get_db)):
