...
from app.db.tables.turns.crud import turn_crud as crud
from app.db.tables.turns.schemas import TurnRead, TurnCreate, TurnOptional
from app.core.schemas import DeleteResponse

router = APIRouter(prefix='/turns',tags=['Turns'])

@router.post('/', response_model=TurnRead, status_code=201)
async def create_turn_endpoint(data: TurnCreate, db: AsyncSession = Depends(get_db)):
    return await crud.create(data, db)

@router.get('/by-id/{id}', response_model=TurnRead)
async def get_by_id_endpoint(id: int, db: AsyncSession = Depends(get_db)):
    return await crud.get_by_id(id, db)

@router.get('/filtered', response_model=List[TurnRead])
...
@router.patch('/{id}', response_model=TurnRead)
async def update_endpoint(id: int, data: TurnOptional, db: AsyncSession = Depends(get_db)):
...
async def delete_endpoint(id: int, db: AsyncSession = Depends(get_db)):
    ...
    return {'msg': 'Turno eliminado correctamente'}
